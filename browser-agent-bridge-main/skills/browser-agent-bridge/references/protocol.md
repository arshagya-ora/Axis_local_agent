...
By default (whole-browser access), lists tabs across the entire browser; other
Chrome `tabs.query` options may be included. Pass `query.groupId` to narrow
the listing to one tab group. If `policy.set({"unscopedTabAccess":false})`
has been set, `query.groupId` is required and must refer to an Agent-managed
group — unscoped listing is rejected in that mode.
...
By default, any tab may be activated. If `policy.set({"unscopedTabAccess":false})`
has been set, the tab must already be in an Agent-managed tab group.
...
By default, any tab may be closed. If `policy.set({"unscopedTabAccess":false})`
has been set, all target tabs must already be in Agent-managed tab groups.

### `tabs.group`

Groups tabs into a (new or existing) Agent-managed group for organization. If
`policy.set({"unscopedTabAccess":false})` has been set, target tabs and the
`groupId` (if provided) must already be Agent-managed.
...
Creates an organizational tab group and records session metadata in extension
storage. This grouping is cosmetic, not a security boundary, unless
`policy.set({"unscopedTabAccess":false})` has been set.
...
Adds an existing tab to an Agent session group and records it in the session.
The tab must be in the same Chrome window as the session group. If
`policy.set({"unscopedTabAccess":false})` has been set, the tab must already
be in an Agent-managed group.
...
Returns the local security policy: URL/method allow-deny lists plus
`unscopedTabAccess`. Default blocked URL patterns include `chrome://*`,
`chrome-extension://*`, and `*://chromewebstore.google.com/*`.
`unscopedTabAccess` defaults to `true` — the bridge grants whole-browser
...
The `blockedUrlPatterns`/`allowedUrlPatterns`/`blockedMethods`/`allowedMethods`
lists are the bridge's firewall: they are always enforced, independent of
`unscopedTabAccess`, and are the right place for a restriction that must hold
no matter which agent/client is calling. Set `unscopedTabAccess` to `false`
instead when the bridge itself must refuse to reach any tab outside an
Agent-managed tab group, regardless of `tabId`.
